...
## Workflow — How to Pick Up Tasks

GitHub Issues are the source of truth for project progress. Every session should start here:

1. **Check open issues**: `gh issue list --state open --limit 20`
2. **Pick the lowest-numbered open issue** — lower numbers have higher priority and earlier issues often unblock later ones
3. **Read the issue body**: `gh issue view <number>` — each has a detailed checklist
4. **Work through the checklist** — commit after completing each logical chunk
5. **Update the issue** as you go — check off completed items, leave comments on decisions or blockers
6. **Close the issue** when all checklist items are done: `gh issue close <number> --comment "summary of what was done"`

The pinned issue **#13 (Roadmap)** has the full overview and dependency map.

### Priority Labels

| Label | Meaning |
|-------|---------|
| `priority:p0` | Must have — blocks MVP |
| `priority:p1` | Should have — core feature |
| `priority:p2` | Nice to have |
| `priority:p3` | Future / low priority |

### MVP Path

The MVP milestone is **Step 4** (issue #5): `gpc auth init` → `gpc deploy --file app.aab --track internal` works end-to-end. Issues #2-#5 are on the critical path.
...
internal/
  ...
  ui/          Terminal mascot and banner rendering (lipgloss + half-block pixel art)
```
...
| lipgloss | `github.com/charmbracelet/lipgloss` | Terminal styling and colors |
